...
if ( ! function_exists( 'seed_comment_count' ) ) :
	/**
	 * Prints HTML with the comment count for the current post.
	 */
	function seed_comment_count() {
		if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			/* translators: %s: Name of current post. Only visible to screen readers. */
			comments_popup_link( sprintf( __( 'Leave a comment<span class="sr-only"> on %s</span>', 'seed3' ), get_the_title() ) );
		}
	}
endif;

if ( ! function_exists( 'seed_entry_meta' ) ) :
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 * This template tag is used in the entry header.
	 */
	function seed_entry_meta() {
		...
		if ( 'post' === get_post_type() ) {
			...
			$categories_list = get_the_category_list( __( ', ', 'seed3' ) );
			if ( $categories_list ) {
				printf(
				/* translators: 1: posted in label, only visible to screen readers. 2: list of categories. */
					'<span class="sr-only">%1$s</span>%2$s',
					esc_html__( 'Posted in', 'seed3' ),
					$categories_list // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				);
			}

			/* translators: used between list items, there is a space after the comma. */
			$tags_list = get_the_tag_list( '', __( ', ', 'seed3' ) );
			if ( $tags_list ) {
				printf(
				/* translators: 1: tags label, only visible to screen readers. 2: list of tags. */
					'<span class="sr-only">%1$s</span>%2$s',
					esc_html__( 'Tags:', 'seed3' ),
		...
		edit_post_link(
			sprintf(
				wp_kses(
				/* translators: %s: Name of current post. Only visible to screen readers. */
					__( 'Edit <span class="sr-only">%s</span>', 'seed3' ),
					array(
						'span' => array(
							'class' => array(),
						),
...
if ( ! function_exists( 'seed_comment_avatar' ) ) :
	/**
	 ...
	 */
	function seed_get_user_avatar_markup( $id_or_email = null ) {

		if ( ! isset( $id_or_email ) ) {
			$id_or_email = get_current_user_id();
		}

		return sprintf( '<div class="vcard">%s</div>', get_avatar( $id_or_email, seed_get_avatar_size() ) );
	}
endif;

if ( ! function_exists( 'seed_discussion_avatars_list' ) ) :
	/**
	 * Displays a list of avatars involved in a discussion for a given post.
	 *
	 * @param array $comment_authors Comment authors to list as avatars.
	 */
	function seed_discussion_avatars_list( $comment_authors ) {
		...
		foreach ( $comment_authors as $id_or_email ) {
			printf(
				"<li>%s</li>\n",
				seed_get_user_avatar_markup( $id_or_email ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			);
		}
		echo '</ol>', "\n";
	}
...
if ( ! function_exists( 'seed_the_posts_navigation' ) ) :
	/**
	 * Wraps `the_posts_pagination` for use throughout the theme.
	 */
	function seed_the_posts_navigation() {
		the_posts_pagination(
			array(
				'mid_size'  => 2,
				'prev_text' => __( 'Newer posts', 'seed3' ),
				'next_text' => __( 'Older posts', 'seed3' ),
			)
		);
	}
endif;

if ( ! function_exists( 'seed_content_class' ) ) :
	/**
	 ...
	 * SEED_TYPOGRAPHY_CLASSES constant.)
	 *
	 ...
	 */
	function seed_content_class( $classes = '' ) {
		$all_classes = array( $classes, SEED_TYPOGRAPHY_CLASSES );
